...
abd_400$Fit <- map2(.x = abd_400$data, .y = titles, function(.x, .y) {
  ...
  parms <- c(mu = max(PCGR, na.rm = T), A = max(PCGR, na.rm = T) / max(y))
  out1 <- ode(y = y0, times = times, func = ltk1, parms = list(A = parms[2], mu = parms[1]))
  ...
  ss2a <- function(parms) {
    ...
    mu <- parms[1]
    # 
    out1 <- deSolve::ode(y = y0, times = times, func = ltk1, parms = list(A = A, mu = mu))
  ...
  de_model <- minpack.lm::nls.lm(par = parms, fn = ss2a)
  ...
  results <- list(Performance = ode_perf, 
                  Estimates = data.frame(Parameter = c("mu", "A"), ode_reults)
                 )
  return(results)
...
plotdata1 <- abd_400 %>% unnest(Estimates)
