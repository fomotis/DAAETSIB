...
source("Codes/Functions/Height_Height.R")
...
source("Codes/Functions/bs4_invasion_c.R")
source("Codes/Functions/bs5_c.R")
source("Codes/Functions/bs5_invasion_c.R")
...
#original particle count
ParticleCount_invasion <- lapply(full_dataset_good_invasion, function(x){
  fsApply(x, nrow)
})
...
######### Margin events for Invasion #############

cellmargin_invasion_special <- function(flow.frame, Channel = "SSC.W") {
    infl_point1 <- flowDensity::deGate(flow.frame, Channel, bimodal = T)
    infl_point2 <- flowDensity::deGate(flow.frame, Channel, after.peak = T)
    #plotting
    flowDensity::plotDens(flow.frame, c(Channel, "FSC.HLin"), main = identifier(flow.frame))
...
#filtering out margin evets
logtrans_margin_invasion <- lapply(log_transformedSet_invasion, function(x) {
  dlist <- fsApply(x, function(y, ch, typ) {
    cellmargin(y, Channel = ch, type = "estimate")$reducedflowframe
  }, ch = "SSC.W")
})

special_invasion <- cellmargin_invasion_special(log_transformedSet_invasion$Day9[[1]])
logtrans_margin_invasion$Day9[[1]] <- special_invasion$reducedflowframe

logtrans_margin_invasion_dataset <- lapply(log_transformedSet_invasion, function(x) {
  fsApply(x, function(y, ch, typ){
    ttt1 <- cellmargin(y, Channel = ch, type = "estimate")
    nmargin <- ttt1$N_margin
    ncell <- ttt1$N_cell
...
logtrans_margin_invasion_dataset$Day9[1, ] <- c(special_invasion$N_cell, special_invasion$N_margin,
                                                special_invasion$N_particle, 
                                                identifier(special_invasion$reducedflowframe))
...
logtrans_debris_percent <- lapply(logtrans_margin, function(x){
  if (length(x) < 36) {
    ...
    counter <<- counter + 1
      if (counter <= 6) {
          ttt <- celldebris(y,  control_use = c(T, T), 
                            interest = "BS4",
    ...
    if(counter <= 18 & substring(identifier(y), first = nchar(identifier(y))) == "1"){
        ttt <- celldebris(y, control_use = c(T, T),interest = "BS4",
        ...
        #counter <<- counter + 1
          ttt <- celldebris(y, control_use = c(T, T),interest = "BS5",
...
counter <- 0
logtrans_debris_percent_invasion <- lapply(logtrans_margin_invasion[c(2, 3:11)],
                                           function(x) {
  #counter2 <<- counter2 + 1
  debris_percentage <- fsApply(x, function(y){
    counter <<- counter+1
    if(counter <= 18 & substring(identifier(y),first = nchar(identifier(y))) == "1") {
        ttt <- celldebris(y, control_use = c(T, T), interest = "Both",
               control_flowFrame = c(BS5_control[[2]][[1]], BS5_control[[2]][[1]]),
               b4_control = BS4_control[[2]][[1]],
               b5_control = BS5_control[[2]][[1]])
        a <- ttt$Cell_count
        b <- ttt$N_particle
        nothers <- ttt$N_others
    ...
    return(data.frame(BS4Count = a[1], BS5Count = a[2], ParticleCount = b, 
                      N_others = nothers, Debriscount = b - (sum(a) + nothers)
                        ))
  })
  counter <<- 0
  debris <- debris_percentage
...
debris_count_invasion_data$OParticleCount <- unlist(ParticleCount_invasion[c(2, 3:11)])
...
debris_count_invasion_data$Dilution[19:21] <- 100
debris_count_invasion_data$Invader <- if_else(debris_count_invasion_data$Resident_Species == 4, 5, 4)

debris_count_invasion_data <- debris_count_invasion_data %>% 
  mutate(BS4Abundance = (BS4Count/OParticleCount) * CPML * as.numeric(Dilution),
 BS5Abundance = (BS5Count/OParticleCount) * CPML * as.numeric(Dilution),
 Resident_Species = as.numeric(Resident_Species),
 Replicate = as.numeric(Replicate))

#merging the datasets together
logtrans_margin_invasion_dataset$Prelim$File <- good_measurements_invasion$Prelim$Filename
logtrans_margin_invasion_dataset <- do.call(rbind.data.frame, 
                                            logtrans_margin_invasion_dataset[c(2, 3:11)])
debris_count_invasion_data <- cbind(debris_count_invasion_data, 
                                    logtrans_margin_invasion_dataset)
debris_count_invasion_data <- debris_count_invasion_data %>% dplyr::select(FileName, Resident_Species,
                                                         Invader, Treatment, Dilution, 
                                                         Time, Replicate, CPML, BS4Count, BS5Count,
                                                         Debriscount, 
                                                         ParticleCount, 
                                                         OtherCount = N_others, 
                                                         MarginCount = nmargin, 
                                                         FParticleCount = nparticle,
                                                         OParticleCount, BS4Abundance, 
                                                         BS5Abundance)
...
write.csv(debris_count_invasion_data, "Data/Abundance_Invasion.csv", row.names = F)
